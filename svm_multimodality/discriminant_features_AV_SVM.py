...
def main(config_module, N_SEED):
    ...
    print("MEAN VALUE OF THE WEIGHTS OF EACH MODALITY")
    permutation_AV_combination = np.asarray(permutation_AV_combination)
    print(np.mean(permutation_AV_combination,axis = 0))
    print("")
    ...
    for m in range(len(data_dirs)):
        ...
        for i, (train_index, test_index) in enumerate(skf.split(y, y)):
            ...
            svc = SVC(kernel='linear', C = best_classifiers[m].C,  probability=True, random_state=1)
        ...
        print(coefs_.shape)
        print("SAVING SVM WEIGHTS IN " + "./results/" + experiment_name + "/AV/SVM_LEARNED_FEATURES" + str(m))
        if type_data[m] == "MATRIX":
            np.savetxt("./results/"+experiment_name+"/AV/SVM_LEARNED_FEATURES"+str(m)+".csv", coefs_, delimiter=',')
        if type_data[m] == "3D":
            empty_header = nib.Nifti1Header()
            another_img = nib.Nifti1Image(coefs_, VOLUME.affine, empty_header)
            nib.save(another_img,"./results/"+experiment_name+"/AV/SVM_LEARNED_FEATURES"+str(m)+".nii")
